"""
Test script for preparation_agent.py
Tests the festival preparation agent to ensure it works correctly.
"""
...
from utsava_agent.agents.preparation_agent import preparation_agent
...
async def test_preparation_agent():
    """Test the preparation agent with a sample festival query."""
    ...
    # Test query
    test_query = "Plan pre-festival activities for Nuakhai in Bhubaneswar, Odisha for a family of 3 with one child. Return pre_festival JSON."
    ...
    try:
        ...
        runner = InMemoryRunner(agent=preparation_agent, app_name="test_preparation")
        ...
        try:
            ...
            # Validate required fields for pre_festival
            required_fields = [
            ...
            for field in required_fields:
                if field in parsed_json:
                    ...
                    if isinstance(value, str):
                        ...
                        if field == "schedule":
                            schedule_fields = ["T-7_days", "T-3_days", "T-1_day"]
                            for sched_field in schedule_fields:
                                if sched_field in value:
                                    logger.info(f"    ✅ schedule.{sched_field}: [{len(value[sched_field])} items]")
                                else:
                                    logger.warning(f"    ⚠️  schedule.{sched_field}: MISSING")
                        # Validate guest_or_travel_plan structure
                        elif field == "guest_or_travel_plan":
                            travel_fields = ["is_travel_suggested", "note"]
                            for travel_field in travel_fields:
                                if travel_field in value:
                                    logger.info(f"    ✅ guest_or_travel_plan.{travel_field}: {value[travel_field]}")
                                else:
                                    logger.warning(f"    ⚠️  guest_or_travel_plan.{travel_field}: MISSING")
                    else:
            ...
            # Validate popular_places_to_visit structure
            if "popular_places_to_visit" in parsed_json:
                places = parsed_json["popular_places_to_visit"]
                if isinstance(places, list):
                    logger.info(f"  Validating {len(places)} places...")
                    for i, place in enumerate(places[:3]):  # Check first 3
                        if isinstance(place, dict):
                            if "place" in place and "suggestion" in place:
                                logger.info(f"    ✅ Place {i+1}: {place.get('place', 'N/A')[:30]}...")
                            else:
                                logger.warning(f"    ⚠️  Place {i+1}: Missing 'place' or 'suggestion' keys")
                        else:
                            logger.warning(f"    ⚠️  Place {i+1}: Not a dict")
        ...
        logger.error(f"❌ Error running preparation agent: {e}", exc_info=True)
        logger.info("=" * 60)
        return False


if __name__ == "__main__":
    success = asyncio.run(test_preparation_agent())
