"""
Test script for experience_agent.py
Tests the festival experience agent to ensure it works correctly.
"""
...
from utsava_agent.agents.experience_agent import experience_agent
...
async def test_experience_agent():
    """Test the experience agent with a sample festival query."""
    
    logger.info("=" * 60)
    logger.info("Testing Experience Agent")
    logger.info("=" * 60)
    
    # Test query
    test_query = "Plan festival day experience for Nuakhai in Bhubaneswar, Odisha for a family of 3 with one child. Return festival_day JSON."
    ...
    try:
        ...
        runner = InMemoryRunner(agent=experience_agent, app_name="test_experience")
        logger.info("Running experience agent...")
        ...
        try:
            ...
            # Validate required fields for festival_day
            required_fields = [
                "early_morning",
                "morning",
                "mid_day",
        ...
        logger.error(f"❌ Error running experience agent: {e}", exc_info=True)
        logger.info("=" * 60)
        return False


if __name__ == "__main__":
    success = asyncio.run(test_experience_agent())
